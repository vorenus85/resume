<template>
  <nav
    class="nav-sidebar"
    :class="{ 'nav-sidebar-shrink': toggled, 'nav-sidebar-svh': utils.isIOS() }"
  >
    <!-- Main Content -->
...
watch(
  ...
  () => {
    if (!settings.value.navToggleEnabled) return;

    if (lastKeyPressed.value.id === "ArrowLeft") emit("toggle", true);
    else if (lastKeyPressed.value.id === "ArrowRight") emit("toggle", false);
  },
);
