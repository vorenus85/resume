<template>
  <div class="timeline-item-content">
    ...
    <div class="timeline-item-content-body">
      <p class="text-4 text-default description m-0" v-html="description" />

      <p
        v-if="list && list.title"
        class="text-4 text-default list-title m-0 mt-2"
        v-html="list.title"
      />

      <ul
        v-if="list && list.items && list.items.length"
      ...
      <div class="tags-wrapper mt-2 pt-1 mt-md-3">
        <Tags v-if="parsedTags && parsedTags.length" :tags="parsedTags" />
      </div>
    </div>
  </div>
</template>
...
const locationString = computed(() => {
  if (!props.province && !props.country) return null;
...
const institutionString = computed(() => {
  if (isScreenXlOrLarger.value) return props.institution;

  const shortenedLocation = props.province || props.country;

  if (!props.institution && shortenedLocation) return locationString.value;
  if (!shortenedLocation && props.institution) return props.institution;
...
const infoListItems = computed(() => {
  ...
  return [
    ...
    .filter((item) =>
      item.visibility.includes(compress ? "mobile" : "desktop"),
    );
});
</script>
